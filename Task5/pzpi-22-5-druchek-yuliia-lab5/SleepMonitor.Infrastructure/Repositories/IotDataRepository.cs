    ...
    : IIotDataRepository
{
    ...
    public async Task<IEnumerable<IoTData?>> GetAllByDateRangeAsync(string userId, DateTime start, DateTime end)
    {
        var iotData = await dbContext.IoTData
            .Where(data => data.UserId == userId && data.Time >= start && data.Time <= end)
            .ToListAsync();
        return iotData;
    }
